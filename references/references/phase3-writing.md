...
**CRITICAL — Citation accuracy**: Every [N] in the draft MUST map to an entry in the Source Inventory in phase2-merged.md. You MUST NOT create reference entries for papers that are not in the Source Inventory. Before writing any claim:

1. **Verify the source exists in the Source Inventory**: check that the paper is listed with a DOI or URL
2. **Verify the reference number**: is [N] the correct inventory entry for this paper?
3. **If a paper is mentioned in Phase 2 agent summaries but has NO Source Inventory entry**: do NOT create a reference for it. Either (a) flag it for Round 2 deep-read verification, or (b) cite only what the Source Inventory supports

**Common failure mode**: Phase 2 agents may mention papers by name ("Sathi et al.", "CAT", "DuPLUS") in their summaries without providing verified DOIs. These names may be wrong (Sathi→Blankemeier) or the papers may not exist. The Source Inventory is the single source of truth — if it's not in the inventory, don't cite it.

**Pre-finalization check**: Before writing the final draft, cross-check every [N] reference number against the Source Inventory. Any reference that does not trace back to an inventory entry with a verified DOI/URL is invalid and must be removed or replaced.
...
Agent B — Citation Completeness (literature-review perspective):
  prompt: |
    Read the draft below. Focus ONLY on citation coverage and existence:
    - Are all factual claims backed by a citation?
    - CRITICAL: For EVERY reference [N] in the draft, verify it exists in the Source Inventory.
      Any reference NOT in the Source Inventory → flag as [UNVERIFIED-SOURCE: not in inventory]
    - Are there missing seminal works in the Source Inventory that should be cited?
    - Are any citations attributed to the wrong paper? (Check author names, years, venues)
    Reference claims by SEMANTIC CONTENT, not line numbers.
    Return a checklist: [MISSING] for missing, [WRONG] for misattributed,
    [UNVERIFIED-SOURCE] for references not in Source Inventory, [OK] for correct.
